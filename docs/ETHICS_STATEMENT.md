...
- **Storage Location:** User data resides **exclusively** in ephemeral session memory (RAM) during the active session.
- **Persistence:** No user input data is written to persistent storage (databases or disk) on the server.
- **Deletion:** All user data is effectively deleted immediately upon session termination or browser window closure.
...
2. **PII Redaction Module**
   - Module: [`modules/pii_redactor.py`](../modules/pii_redactor.py)
...
### Anonymization Caveats
> [!WARNING]
> **Mosaic Effect:** While the system employs PIIRedactor to remove direct identifiers (Name, Email, etc.), we acknowledge that effective anonymization is complex. Re-identification risks (e.g., via mosaic effects, linguistic fingerprinting, or specific career history combinations) can never be reduced to zero for rich documents like CVs. **Therefore, our primary defense is Data Minimization: we do not store or redistribute raw user text at all, relying solely on aggregated scoring metrics.**
...
> "Your CV is processed locally in temporary memory and is not stored or shared. All data is deleted when you close this session."
...
2. **Bias in Scoring Algorithms**
   ...
   - **Mitigation:** Transparent scoring equations (see [`docs/scoring_equations.md`](./scoring_equations.md))
...
**Current State:** LICENSE file exists (CC0 1.0 Universal)  
**Ethical Concern:** Resolved (dedicated to public domain)

### Dataset License

**Status:** **Definitive (CC0 1.0 Universal)**  
**Ownership:** Project authors affirm ownership of synthetic outputs (generated via OpenAI).  
...
- ✅ Scoring equations documented (see [`docs/scoring_equations.md`](./scoring_equations.md))
...
| **Licensing clarity** | ✅ Verified | CC0 1.0 Universal |
